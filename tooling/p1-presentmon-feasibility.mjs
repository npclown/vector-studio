...
try {
  for (const channel of ['chrome', 'msedge']) {
    ...
    try {
      ...
      const ownership = JSON.parse(
        execFileSync(
          ...
          [
            '-NoProfile',
            '-Command',
            `Get-CimInstance Win32_Process -Filter 'ProcessId = ${target}' | Select-Object ProcessId,ParentProcessId,Name | ConvertTo-Json`,
          ],
      ...
      record.startedUtc = new Date().toISOString();
      record.captureAttempted = true;
      capture = spawn(tool, args, { windowsHide: true, timeout: 30000 });
      let stdout = '',
        stderr = '';
      ...
      let ended = false;
      const completion = once(capture, 'close').then(([code, signal]) => {
        ended = true;
        return { code, signal };
      });
      const start = Date.now();
      while (!ended && Date.now() - start < 30000) {
        await page.mouse.move(40 + ((Date.now() - start) % 500), 180);
        await delay(100);
      }
      if (!ended) {
      ...
      record.processExitSucceeded = record.exit.code === 0;
      ...
      if (!record.acquisitionValid) {
        process.exitCode = 2;
        record.remainingBrowser = 'NOT RUN: acquisition precondition failed; no elevation or retry';
        break;
      }
    } catch (error) {
