...
## Authorized elevated retry: 2026-09-12

The user explicitly approved the proposed one-time elevated PresentMon retry after the access-denied report. Only the same hash-pinned executable may request `RunAs`; Node, the PowerShell launcher and isolated browsers remain unprivileged. Keep one capture per Chrome/Edge, `--timed 25`, termination after the timer/producer exit and a 30-second post-launch watchdog. UAC consent wait is recorded separately from capture time; it cannot be silently counted as a completed capture. No permanent group membership, driver/service installation, existing-session termination or acceptance change is authorized.

Before capture, statically validate the helper and independently review the elevation, PID and cleanup paths. Recheck the same GPU creation identity/parent and page window after consent. Preserve original source hashes, CSV, markers, helper PID/exit/timeout records and any failure under a fresh run identity. `RunAs` cannot redirect child stdout/stderr; record that limit rather than treating launcher streams as PresentMon output. A timeout, changed producer or remaining owned process invalidates acquisition and stops the pair. Local checks/build and Primary review precede the checkpoint PR. This retry only tests acquisition feasibility; clock/content correlation, A09 and A10 remain separate unresolved gates.

Outcome: the [new immutable review](../evidence/p1.0m/20260912-presentmon-elevated/README.md) records a prelaunch hash-command failure, one successful 25-second Chrome ETW capture using the emitted default/non-v2 schema, and cancellation of the remaining Edge UAC request before PresentMon creation. The original Chrome missing-v2-columns result is preserved; offline schema review confirms acquisition only. The helper now requests explicit v2 columns for future use, but Edge produced no CSV. No Chrome repeat, new consent request, pooled timing, acceptance relaxation or product implementation followed. P1.0m remains PARTIAL with A09/A10 UNVERIFIED.
