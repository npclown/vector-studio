...
### D1 — proposed scene and packet contract

**Approval required before exports or implementation:** this adds a meaningful scene-facing API where only lifecycle interfaces exist today. The semantics and exported TypeScript shape below are the approval proposal. Internal packet byte layouts remain Primary-owned implementation decisions; meaningful changes to this public shape require a new decision.
...
### D2 — proposed P1 visual behavior

**Approval required before changing the owning graphics design:** color-space and opacity semantics affect visible output, and the current architecture explicitly leaves them open.
...
- For P1, structural containers must have opacity 1; non-unit container opacity is rejected atomically as unsupported, never approximated by multiplying descendant alpha. The proposed eventual behavior is isolated group composition, requiring a later owning design/plan before support. This is a P1 validation boundary, not removal of the MVP opacity requirement. The user may instead require isolated container opacity in P1; that needs a revised work breakdown for offscreen composition before implementation.
...
All implementation criteria below are **TODO**. A draft acceptance description is not evidence. `P1-A10` is specifically **UNVERIFIED — measurement method unresolved**.
...
The roadmap owns the four scenes and their thresholds. P1.0m must freeze a versioned configuration and instrumented-memory/event method before product implementation; no threshold is selected after observing measurements.

- Proposed reference: production build, current Windows reference machine, stable Chrome/Edge, 1280x720 physical at DPR 1, preferred format and actual sample count recorded, verified 60 Hz monitor/window association. Recheck power/background load before running. A different display rate requires method review, not a silently rescaled threshold.
- Proposed sampling: five fresh repetitions per browser/scenario, 5 s warm-up + 10 s measured continuous interaction per repetition. Preserve all frame intervals, submit times, write ranges/bytes, geometry rebuilds, visible/submitted counts, pipeline counts, errors and availability. Nearest-rank percentiles; use maximum per-repetition p95, not pooled p95. Scene 1/3 ceiling stays 16.7 ms; scene 2 stays 33.3 ms. Define frame-time observation explicitly in the P1.0m method review; P0's RAF interpretation and 18 ms gate are not automatically imported into P1.
- Workload seed `0x50310001`, versioned deterministic generator and hash of full configuration. Equal modulo-four distribution of rectangle, rounded rectangle, ellipse and line. P1.0m must fix exact coordinates, dimensions, stroke/alpha mix, paint order, target node, pan/zoom trajectory and event cadence in the stored configuration. Scene 1/2 must remain all-visible throughout their trajectory; scene 3 records actual visible count and freezes a defensible approximately-1,000 range before runs. Scene 4 moves one fixed primitive without hierarchy/viewport changes, records actual dirty and native upload ranges, and must not hide full uploads behind only byte-count averages.
- Combined memory is decimal 256 MB, not 256 MiB. Sum simultaneously live durable fixture/document storage, accepted mirror/index/order/style storage, CPU geometry and upload buffers, WASM linear memory (zero only while no WASM exists), and engine-accounted GPU allocations. Count copies separately, shared backing stores once, allocator backing capacity rather than just live slices, and transient old/new buffers and recovery/resize overlaps. Count instrumented memory consistently, and state whether measurement overhead is included or separately bounded.
- Browser/driver internals, browser executable, unrelated page content and test-framework machinery are excluded and reported as exclusions. JS object/map/string overhead is **not** silently excluded from engine CPU memory. P1.0m must validate a conservative inventory/heap method for that overhead and coordinate CPU/GPU peak observations. Encoded JSON length, typed-array capacity alone, unrelated browser heap totals or a GPU-only counter cannot prove the combined ceiling. If unavailable, A09 remains UNVERIFIED.
- Record input event timestamps and the scene/frame revision that first reflects each input; account for coalesced and superseded inputs rather than selecting only fast samples. The endpoint, clock mapping, sample eligibility, uncertainty and required sample count for A10 remain blocked on a measurement method. Recording queue completion as a diagnostic does not unblock it.
...
Primary source review corrected the draft's claim that the existing backend already owns submission serials, clarified local-space affine strokes and the task DAG, and added the exact proposed public synchronization and camera surface, including independent camera revisions and coalesced viewport invalidation. Critical review moved unresolved measurement contracts ahead of all implementation, closed same-identity snapshot replay/rollback and container-opacity validation gaps, and fixed the encoded-sRGB target and local fill/stroke/opacity formula. No proposal was promoted to an accepted architecture decision.
...
D1/D2 are pending; P1.0b and every implementation task remain unstarted. A draft PR does not authorize those decisions or imply the P1 exit gate passes.
