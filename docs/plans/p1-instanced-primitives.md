...
P1-A01 is PASS at the CPU scene-synchronization boundary. A02/A04 retain their CPU evidence, extended by P1.5 native precision/culling checks. A03/A06/A07 are PASS with combined [P1.4 unit evidence](../evidence/p1.4-integration-review-2026-09-12.md) and [P1.5 headed evidence](../evidence/p1.5-visual-review-2026-09-12.md). A05 remains PARTIAL pending scene-4 production records; A08 is TODO. A09/A10 remain UNVERIFIED with unresolved measurement methods; the complete P1 exit gate has not passed.
...
| P1-A03 | Rectangles, independent radii, ellipses and simple lines satisfy D2, overlap order and declared numeric/visual tolerances in Chrome and Edge.                                                                                                                      | PASS: [P1.3 component review](../evidence/p1.3-analytic-review-2026-09-12.md) + [P1.5 headed corpus](../evidence/p1.5-visual-review-2026-09-12.md) |
...
| P1-A05 | Shared unit geometry is reused; color/opacity edits rebuild zero geometry. A warmed isolated transform edit rebuilds zero geometry, uploads no unrelated instance/style records and never uploads a full scene buffer. Camera-only geometry rebuild count is zero. | PARTIAL: P1.2/P1.4 unit ranges and P1.5 headed warm/edit counters; P1.7 scene-4 production records remain required                                 |
| P1-A06 | Ordered adjacent batching preserves D2 overlap output. No new shader/pipeline during warmed unchanged frames. Exactly one scheduler/device lifetime owner exists.                                                                                                  | PASS: P1.4 packet/lifecycle doubles + [P1.5 overlap and warm pipeline evidence](../evidence/p1.5-visual-review-2026-09-12.md)                      |
| P1-A07 | Loss rebuilds latest accepted mirror; edits while recovering survive; no stale submission/acknowledgment or premature allocation reuse. Partial failures and dispose release owned resources.                                                                      | PASS: P1.4 delayed native/lifetime doubles + [P1.5 latest-scene recovery](../evidence/p1.5/20260912-recovery/README.md)                            |
...
### P1.5 checkpoint (2026-09-12)

Start: clean fetched `main` at `c3e1246` (P1.4, PR #34). Branch: `codex/p1-5-headed-primitives`. The checkpoint retains the frozen V01-V07 and full N02 Cartesian corpus, 640x360 CSS, DPR 1/1.5/2, separate 1x/4x, 2/255 interior/exterior color tolerance, one-physical-pixel contour tolerance and 0.25-physical-pixel visible position budget. No performance claim or A09/A10 method is added.

The separate playground fixture composes the existing service/backend. Test-only device instrumentation requests the existing 1x fallback, changes clear alpha for V02/V03, enables canvas COPY_SRC, and captures raw premultiplied target bytes before the current texture expires. The production shader, packet consumption and lifecycle remain unchanged. PNG encoding explicitly unpremultiplies the separately retained raw readback; numerical assertions use raw premultiplied bytes. Sequential fixture loads use monotonically increasing snapshot revisions with the frozen document/page identity and unchanged literal geometry/camera values.

An additional test-only native position probe executes the captured production vertex body in the vertex stage, then relocates its output into four float readback cells. Independent f64 scene/camera construction checks both the local guarded corners and their physical projections, then compares visible clipped contour/guard points with piecewise affine interpolation using independently constructed quad weights. The wrapper copies packet byte views synchronously before returning; the asynchronous probe owns its copies. This is native vertex arithmetic evidence, not a claim of fragment-stage position readback; unmodified shader raster images and their independent contour oracle remain separately required.

The image oracle treats the geometric edge as the local region-color transition (nearest premultiplied region color, equivalent to a 50% contrast classification between two regions). It measures wrong-region samples against transformed analytic contours within the unchanged one-pixel threshold; low-coverage AA fringe is not itself geometric displacement. Exact color assertions remain at pixel centers conservatively at least two physical pixels from all color contours, with tolerance 2/255. Missing visible geometry is checked separately so thin lines cannot pass through absence of stable interior samples. Raw RGBA gzip files, full PNGs, bounded color/classification records and the deterministic sample algorithm are retained for reproduction.

Acceptance evidence must record all combinations, exact browser versions/adapter, source manifest, raw sampled coordinates/colors, full PNG paths, errors/diagnostics, warm upload and pipeline behavior, latest-revision recovery images, rejection preservation and disposal accounting. Exploratory failures stay in distinct run directories. A failed oracle is repaired against its mathematical contract and mutation tests; a renderer failure must be repaired without changing thresholds. Primary reviewed the complete matrix and corrected recovery successor; the [review](../evidence/p1.5-visual-review-2026-09-12.md) records the source deltas, failed attempts, accepted evidence and remaining gate.

Actual delegation: Terra medium supplied the frozen corpus and an initial oracle; Primary rejected the weak oracle and escalated its correction to Sol medium. Sol also supplied the disjoint native position probe and read-only review. Luna low executed static/build/browser commands and archive hash checks. Primary corrected corpus, guard/byte ownership, recovery fixtures and evidence handling, and owns acceptance. No recursive delegation or concurrent GPU runs occurred; no dependency, public API or runtime scheduler changed.

Local outcome: `pnpm check` PASS (183 tests/26 files), `pnpm build` PASS (37 playground modules), `pnpm test:browser` PASS (12). The full GPU command recorded 34 passed/four recovery-harness failures, with 240 visual and 6,048 numeric fixtures valid; final `pnpm test:gpu --grep 'P1 (visual|latest-scene)'` PASS (16 tests) repeats all visual combinations and corrected recovery. Full archives and explicit source-reuse analysis are linked from the review. P1.5 is complete locally; required PR checks remain separate. Next: P1.0m A09/A10 method resolution before P1.6.
